...
- With the default `session_reuse: true`, Claude, Codex, Grok, Pi, Antigravity, and any ACP target advertising `loadSession` reuse one durable fixer session across review-fix turns; a resume failure retries the same fix turn in a fresh fixer session, and unsupported agents run cold
