...
When enabled and the pipeline agent supports native session resume (Claude or Grok via `--resume`, Codex via `exec resume`, Pi via `--session <UUID>`, Antigravity via `--conversation <id>`, and an ACP target via ACP's own `session/load`), each run keeps one durable fixer session across its review-fix turns.
An ACP target resumes only when it advertises the `loadSession` capability in its `initialize` response - Cursor (`acp:cursor`) does; a target that does not advertise it never has an identity recorded and keeps running every turn cold.
